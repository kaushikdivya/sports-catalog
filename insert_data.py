...
try:
    ...
    cur.execute("CREATE TABLE category_items (id SERIAL PRIMARY KEY, user_id INTEGER REFERENCES user_info(id), category_id INTEGER REFERENCES category(id), name VARCHAR(25) NOT NULL, description text NOT NULL, created_time TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT clock_timestamp(), modified_time TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT clock_timestamp(), expiry_date TIMESTAMP WITH TIME ZONE)")
